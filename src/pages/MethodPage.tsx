...
const PIPELINE = [
  {
    ...
    text: 'Историческое фото и современная съёмка одного фасада выбираются экспертом. Важно, чтобы была понятна плоскость фасада и видимые опорные точки.',
  },
  {
    step: '2',
    title: 'Подготовить фотоматериалы в Archiview',
    text: 'Для близких ракурсов фасад можно выпрямить и сопоставить в overlay. Если ракурсы разные, как у Кривоколенного, Archiview используется для side-by-side-разметки без приведения фото к одной плоскости.',
  },
  {
    step: '3',
    title: 'Сравнить слои',
    text: 'Overlay нужен, когда фотографии можно корректно совместить. Side-by-side нужен, когда достаточно идентифицировать элемент на разных снимках: например, есть вывеска или она не читается.',
  },
  {
    step: '4',
    title: 'Разметить следы памяти',
    text: 'Эксперт обводит зоны изменений: надстройки, заложенные проёмы, утраты декора, вывески, мемориальные слои.',
  },
  {
    step: '5',
...
const FILES = [
  ...
  ['facade-project.json', 'матрица переноса координат для overlay или параметры side-by-side'],
  ['marked-facade.png', 'основная картинка фасада с номерами'],
  ['historical-rectified.png / modern-rectified.png', 'пара фотоматериалов; для side-by-side это не обязательно выпрямленные ракурсы'],
  ['side-by-side-marked.png', 'разметка разных ракурсов для вывесок и сложных случаев'],
]
...
const LIMITS = [
  ...
  'Исторические выводы требуют источников и экспертной проверки; сайт не заменяет экспертизу.',
]
...
const EXPERT_LINKS = BUILDINGS.map((building) => ({
  title: building.name,
  cardId: building.cardId,
  to: `/expert/${building.id}`,
...
export function MethodPage() {
  return (
    <div className="space-y-6">
      ...
      <MethodSection title="Как сайт защищается от съехавшей подсветки" kicker="Контроль качества">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-xl border border-arch-line bg-arch-green-soft p-4">
            ...
            <p className="mt-2 text-sm text-arch-ink/80">
              Для разных ракурсов используется отдельная разметка панелей без выпрямления в общую
              плоскость. Так можно честно показать, есть ли элемент на каждом снимке.
            </p>
          </div>
        </div>
      ...
      <MethodSection title="Экспертная проверка подсветок" kicker="Служебный доступ">
        <p className="mb-4 max-w-3xl text-sm leading-relaxed text-arch-muted">
