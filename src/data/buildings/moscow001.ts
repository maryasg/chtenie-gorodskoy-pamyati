import type { Building } from '../../types/building'

const EXPERTISE_ACT_URL =
  'https://www.mos.ru/upload/documents/files/6285/BolshayaOrdinkayld17str1kv13d17str1d19str1(chast)Pyatnickayayld14str1213d12str4(31052019).pdf'

const EXPERTISE_SOURCE = 'Акт историко-культурной экспертизы (mos.ru, 2019)'

export const MOSCOW_001: Building = {
  id: 'MOSCOW_001_kumaninykh',
  cardId: 'MOSCOW_001',
  name: 'Усадьба Куманиных / Дом Ардовых',
  address: 'Москва, Большая Ордынка, 17, стр. 1',
  lat: 55.7425,
  lng: 37.6258,
  mapStatus: 'verified',
  cardVersion: '0.2',
  style: 'Палаты XVIII / усадьба XVIII–XIX',
  yearBuilt: 'XVII–XIX',
  headline: 'Палаты XVII века, перестроенные в усадьбу Куманиных → дом Ардовых; Ахматова, Янковский, Баталов',
  methodologyNote: 'Слоистый фасад: древние палаты, перестройки под усадьбу, мемориальные слои.',
  protectionStatus: 'памятник архитектуры регионального значения',
  summary:
    'Здание выросло из палат XVII–XVIII веков, которые Куманины перестроили под свою усадебную застройку; позже здесь сформировался дом Ардовых. Характерный «слоистый» дом центра Москвы. В разное время здесь жили и работали Ахматова, Янковский, Баталов. Рядом — тяжёлая память о зоне принудработ у Иоанно-Предтеченской обители.',
  verification: {
    historicalPhoto: true,
    historicalPhotoYear: '1924',
    modernPhotoYear: '2026',
    officialExpertise: [
      {
        title: 'Акт историко-культурной экспертизы (Большая Ордынка, 17, стр. 1)',
        url: EXPERTISE_ACT_URL,
        issuedAt: '2019',
      },
    ],
  },
  memoryTraces: [
    {
      id: 'T001',
      type: 'layer_seam',
      title: 'Швы слоёв застройки',
      period: 'XVIII–XX',
      confidence: 'confirmed',
      overallConfidence: 0.92,
      userMessage:
        'В кладке и ритме фасада читаются палаты XVIII века, усадебные перестройки XIX века и поздние вмешательства XX века.',
    },
    {
      id: 'T002',
      type: 'added_floor',
      title: 'Советская надстройка этажей (1938)',
      period: '1938',
      confidence: 'confirmed',
      overallConfidence: 0.95,
      userMessage:
        'Двухэтажная усадебная часть надстроена до пяти этажей; верхние ярусы получили облик типового советского жилого дома. На фасаде виден шов между старым объёмом и надстройкой.',
    },
    {
      id: 'T003',
      type: 'bricked_opening',
      title: 'Заложенные и изменённые проёмы',
      period: 'XIX–XX',
      confidence: 'confirmed',
      overallConfidence: 0.9,
      userMessage:
        'Часть окон заложена, появились новые проёмы и переделанные входы — типичный след многослойной городской застройки и поздних ремонтов.',
    },
    {
      id: 'T004',
      type: 'changed_entrance',
      title: 'Вход, переделанный в окно (нижний этаж)',
      period: 'XX в.',
      confidence: 'confirmed',
      overallConfidence: 0.93,
      userMessage:
        'На сравнении с планом усадьбы 1840 года здесь был вход; на современном фасаде — оконный проём. Типичная переделка при коммунальном и квартирном использовании дома. Подтверждается актом историко-культурной экспертизы (mos.ru, 2019).',
    },
    {
      id: 'T005',
      type: 'new_balcony',
      title: 'Новый балкон на втором этаже',
      period: 'XX в.',
      confidence: 'confirmed',
      overallConfidence: 0.95,
      userMessage:
        'На втором этаже появился балкон, не характерный для первоначальной усадебной части. Подтверждено визуальным материалом (архивное и современное фото, разметка Archiview).',
    },
    {
      id: 'T006',
      type: 'memorial_sign',
      title: 'Мемориальные доски и память места',
      period: 'современность',
      confidence: 'confirmed',
      overallConfidence: 0.98,
      userMessage:
        'Доски Ахматовой, Баталова и другие мемориальные знаки — современный слой памяти о жильцах и гостях дома (в т.ч. кв. 13, семья Ардовых).',
    },
    {
      id: 'T007',
      type: 'new_window',
      title: 'Окно на надстроенном этаже (левая часть)',
      period: '1938',
      confidence: 'confirmed',
      overallConfidence: 0.9,
      userMessage:
        'Проём в теле советской надстройки 1938 года. Оформление упрощено по сравнению с дореволюционными окнами ниже — без усадебного декора.',
    },
    {
      id: 'T008',
      type: 'new_window',
      title: 'Окно на надстроенном этаже (центр)',
      period: '1938',
      confidence: 'confirmed',
      overallConfidence: 0.9,
      userMessage:
        'Окно верхнего яруса, появившееся при надстройке до пяти этажей. Верхние этажи получили облик типового советского жилого дома.',
    },
    {
      id: 'T009',
      type: 'new_window',
      title: 'Окно на надстроенном этаже (правая часть)',
      period: '1938',
      confidence: 'confirmed',
      overallConfidence: 0.9,
      userMessage:
        'Проём на правом участке надстроенного объёма. Контраст с нижними усадебными этажами наглядно показывает шов 1938 года.',
    },
    {
      id: 'T010',
      type: 'new_window',
      title: 'Новый проём входа (появился к 1924 году)',
      period: 'до 1924',
      confidence: 'confirmed',
      overallConfidence: 0.88,
      userMessage:
        'Вход, которого не было на плане усадьбы 1840 года, уже виден на фотографии 1924 года — след очередной перестройки между дореволюционными слоями и советской надстройкой.',
    },
    {
      id: 'T011',
      type: 'filled_window',
      title: 'Заложенное окно (левый участок фасада)',
      period: 'XIX–XX в.',
      confidence: 'confirmed',
      overallConfidence: 0.88,
      userMessage:
        'Проём заложен кирпичом при перестройках усадьбы или поздних ремонтах. След многослойной застройки, зафиксированный в акте экспертизы.',
    },
    {
      id: 'T012',
      type: 'filled_window',
      title: 'Заложенное окно (центральная часть)',
      period: 'XIX–XX в.',
      confidence: 'confirmed',
      overallConfidence: 0.88,
      userMessage:
        'Окно усадебного периода заложено — типичная переделка при изменении планировки и коммунальном использовании дома.',
    },
    {
      id: 'T013',
      type: 'filled_window',
      title: 'Заложенное окно (правее центра)',
      period: 'XIX–XX в.',
      confidence: 'confirmed',
      overallConfidence: 0.88,
      userMessage:
        'Заложенный проём на фасаде палатно-усадебного слоя. На архивных снимках и в экспертизе читается как след перестроек.',
    },
    {
      id: 'T014',
      type: 'filled_window',
      title: 'Заложенное окно (правая часть)',
      period: 'XIX–XX в.',
      confidence: 'confirmed',
      overallConfidence: 0.88,
      userMessage:
        'Ещё один заложенный проём — часть общей картины изменённых окон и входов на фасаде Ордынки, 17.',
    },
    {
      id: 'T015',
      type: 'filled_window',
      title: 'Заложенное окно (нижний ярус)',
      period: 'XIX–XX в.',
      confidence: 'confirmed',
      overallConfidence: 0.88,
      userMessage:
        'На нижнем этаже проём заложен при приспособлении здания под жилое использование. Согласуется с описанием «следов заложенных проёмов» в акте экспертизы.',
    },
    {
      id: 'T016',
      type: 'changed_entrance',
      title: 'Заложенный вход, расширен в широкое окно',
      period: 'XX в.',
      confidence: 'confirmed',
      overallConfidence: 0.91,
      userMessage:
        'На плане усадьбы 1840 года здесь был вход; позже проём заложили и устроили широкое оконное отверстие большего размера — след коммунальных переделок. Видно на сравнении 1840 → 2026.',
    },
  ],
  artifacts: [
    {
      id: 'art-palaty',
      title: 'Палаты середины XVIII века',
      period: 'XVIII в.',
      confidence: 'confirmed',
      location: 'центральная дворовая секция',
    },
    {
      id: 'art-fence',
      title: 'Ограда вдоль Ордынки',
      period: 'XIX в.',
      confidence: 'confirmed',
      location: 'линия улицы',
    },
    {
      id: 'art-memorial',
      title: 'Мемориальные таблички',
      period: 'XX–XXI',
      confidence: 'confirmed',
    },
  ],
  timeline: [
    {
      id: 'tl-1',
      period: 'середина XVIII в.',
      title: 'Палатный слой',
      whatChanged: 'Сформирована центральная часть из палат середины XVIII века',
      visibleToday: 'Старые объёмы и кладка в центральной секции, «дворовой» характер застройки',
      confidence: 'confirmed',
      source: EXPERTISE_SOURCE,
    },
    {
      id: 'tl-2',
      period: 'конец XVIII — XIX в.',
      title: 'Усадьба Куманиных',
      whatChanged:
        'Дом расширен и перестроен под купеческую усадьбу; после пожара 1812 года — очередная реконструкция',
      visibleToday: 'Слоистая кладка, следы заложенных проёмов, усадебная ограда',
      confidence: 'confirmed',
      source: EXPERTISE_SOURCE,
    },
    {
      id: 'tl-3',
      period: '1938',
      title: 'Советская надстройка',
      whatChanged:
        'К усадебному объёму добавлены этажи; здание приобрело облик пятиэтажного жилого дома конструктивизма / раннего сталинского периода',
      visibleToday: 'Верхние этажи без усадебного декора, шов между старым и новым объёмом',
      confidence: 'confirmed',
      source: EXPERTISE_SOURCE,
    },
    {
      id: 'tl-4',
      period: 'XX — XXI в.',
      title: 'Коммунальные переделки и память',
      whatChanged:
        'Переделки входов и проёмов, балкон на 2-м этаже; мемориальные таблички (Ахматова, Баталов и др.), память о кв. 13',
      visibleToday: 'Изменённые входы, новые окна, балкон на втором этаже; мемориальные знаки на фасаде и во дворе',
      confidence: 'confirmed',
      source: EXPERTISE_SOURCE,
    },
  ],
  photos: [
    {
      id: 'pastvu-2517403',
      type: 'archive',
      description: 'Архив PastVu (1920-е)',
      url: 'https://pastvu.com/p/2517403',
    },
    { id: 'shm', type: 'archive', description: 'ГИМ', url: 'https://catalog.shm.ru/entity/OBJECT/3106660' },
    {
      id: 'current',
      type: 'facade',
      description: 'Современный фасад (полевая съёмка)',
      status: '2026',
    },
  ],
  hotspots: [],
  sources: [
    {
      id: 'S001',
      name: 'Акт историко-культурной экспертизы (PDF, mos.ru)',
      url: EXPERTISE_ACT_URL,
    },
    { id: 'S002', name: 'Узнай Москву', url: 'https://um.mos.ru/houses/usadba_kumaninykh/' },
    { id: 'S003', name: 'PastVu', url: 'https://pastvu.com/p/2517403' },
    { id: 'S004', name: 'ГИМ', url: 'https://catalog.shm.ru/entity/OBJECT/3106660' },
  ],
}
