...
APP_VERSION = "v16 polygon edit + stable site links"
...
def _point_dist2(a: Point, b: Point) -> float:
    dx = a[0] - b[0]
    dy = a[1] - b[1]
    return dx * dx + dy * dy


def _closest_point_on_segment(p: Point, a: Point, b: Point) -> Tuple[float, Point]:
    ax, ay = a
    bx, by = b
    px, py = p
    abx = bx - ax
    ...
    if ab_len2 < 1e-12:
        return _point_dist2(p, a), a
    t = max(0.0, min(1.0, ((px - ax) * abx + (py - ay) * aby) / ab_len2))
    cx = ax + t * abx
    cy = ay + t * aby
    return _point_dist2(p, (cx, cy)), (cx, cy)
...
class AppV13(AppV12):
    ...
    def _build_ui(self) -> None:
        ...
        title = ttk.Label(
            self,
            text="Archiview CV v16: источники → выпрямление → сравнение → разметка",
            font=("TkDefaultFont", 12, "bold"),
        )
        title.pack(anchor="w", padx=12, pady=(10, 4))
        ...
        self.markup_canvas.bind("<Configure>", lambda _e: self._schedule_markup_refresh(delay=120))
        self.result_canvas.bind("<Configure>", lambda _e: self._schedule_result_refresh(delay=120))
        self._log("v16: откройте дом в «Мои проекты» или добавьте новый — дальше PastVu и 4 угла на этой вкладке.\n")
