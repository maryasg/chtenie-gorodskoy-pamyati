...
Preview-деплой использует тот же GitHub Pages сайт, что и основной сайт.

**Раньше:** если после preview запускался deploy из `main`, папка `pr-preview/...` исчезала и ссылка открывалась пустой.

**Сейчас (после обновления workflow):** deploy из `main` снова собирает preview для всех открытых PR с ветками `cursor/...` и кладёт их рядом с основным сайтом.

Если preview всё же пропал:

1. сделайте любой push в ветку PR (или закройте/откройте PR);
2. либо GitHub → Actions → **Deploy preview to GitHub Pages** → Run workflow → `preview_ref` = ваша ветка, `pr_number` = номер PR.
